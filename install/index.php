...
if($_POST) {
	...
	if($core->validate_post($_POST) == true)
	{
      ...
      echo "<h1> Please edit <red>../application/config/config.php</red> to complete basic configuration</h1><br>";
      echo "<b> Also remember to delete the <i>/install</i> folder</b><br>";
			echo "<a href='$redir'> go Home</a>";
			file_put_contents("../application/config/credentials.php","<?php 
				//access credentials
 \$access_user= '{$_POST['user']}';
 \$access_pass	= '{$_POST['pass']}';
 \$master_pass	= '{$_POST['mast']}';");
			die();
		}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>Install | eventList 1.0</title>
    ...
    <?php if(is_writable($db_config_path)){?>
        ...
        <fieldset>
          ...
          <hr>
          <h2>Access credentials for admin panel</h2>
          <label for="user">Username</label><input type="text" id="user" class="input_text" name="user" />
          <label for="pass">Password</label><input type="password" id="pass" class="input_text" name="pass" />
          <label for="mast">Password</label><input type="password" id="mast" class="input_text" name="mast" />
